...
impl CounterSampleBufferDescriptor {
    pub fn new() -> Self {
        unsafe {
            let allocated = msg_id(class(b"MTLCounterSampleBufferDescriptor\0"), sel(b"alloc\0"));
            Self {
...
impl Device {
    ...
    pub fn new_counter_sample_buffer(
        ...
        unsafe {
            ...
            if !responds_to_selector(self.raw, selector) {
                return Err(MetalError::new("newCounterSampleBufferWithDescriptor:error: is not supported"));
            }
            let mut error = NIL;
            let raw = msg_id_id_err(self.raw, selector, descriptor.raw, &mut error);
            if raw.is_null() {
                Err(MetalError::new(error_message(error, "failed to create counter sample buffer")))
            } else {
...
impl RenderCommandEncoder {
    pub fn sample_counters_in_buffer(
        ...
        unsafe {
            ...
            if !responds_to_selector(self.raw, selector) {
                return Err(MetalError::new("sampleCountersInBuffer:atSampleIndex:withBarrier: not supported on this RenderCommandEncoder"));
            }
            let f: unsafe extern "C" fn(id, SEL, id, usize, BOOL) = transmute(objc_msgSend as *const c_void);
            f(self.raw, selector, sample_buffer.raw, sample_index, if barrier { YES } else { NO });
            Ok(())
        }
    }
}
...
impl ComputeCommandEncoder {
    pub fn sample_counters_in_buffer(
        ...
        unsafe {
            let selector = sel(b"sampleCountersInBuffer:atSampleIndex:withBarrier:\0");
            if !responds_to_selector(self.raw, selector) {
                return Err(MetalError::new("sampleCountersInBuffer:atSampleIndex:withBarrier: not supported on this ComputeCommandEncoder"));
            }
            let f: unsafe extern "C" fn(id, SEL, id, usize, BOOL) = transmute(objc_msgSend as *const c_void);
            f(self.raw, selector, sample_buffer.raw, sample_index, if barrier { YES } else { NO });
            Ok(())
...
impl BlitCommandEncoder {
    pub fn sample_counters_in_buffer(
        ...
        unsafe {
            let selector = sel(b"sampleCountersInBuffer:atSampleIndex:withBarrier:\0");
            if !responds_to_selector(self.raw, selector) {
                return Err(MetalError::new("sampleCountersInBuffer:atSampleIndex:withBarrier: not supported on this BlitCommandEncoder"));
            }
            let f: unsafe extern "C" fn(id, SEL, id, usize, BOOL) = transmute(objc_msgSend as *const c_void);
            f(self.raw, selector, sample_buffer.raw, sample_index, if barrier { YES } else { NO });
            Ok(())
    ...
    pub fn resolve_counters(
        ...
        unsafe {
            ...
            if !responds_to_selector(self.raw, selector) {
                return Err(MetalError::new("resolveCounters:inRange:destinationBuffer:destinationOffset: not supported on this BlitCommandEncoder"));
            }
            let f: unsafe extern "C" fn(id, SEL, id, Range, id, usize) = transmute(objc_msgSend as *const c_void);
            f(self.raw, selector, sample_buffer.raw, range, destination_buffer.raw, destination_offset);
            Ok(())
        }
    }
}
